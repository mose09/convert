...
_SOURCE_MIN_SCORE = 5             # 이 이상이어야 매칭으로 인정 (오매칭 방지)
...
def _match_source(asis_stem: str, source_index: list[Path]) -> Path | None:
    """캡처 stem 에 가장 잘 맞는 소스 파일 1개. 임계점수 미만이면 None."""
    if not source_index:
        return None
    ...
    best: Path | None = None
    best_score = 0
    for cand in source_index:
        score = _score_match(stem_tokens, cand)
        if score > best_score:
            best_score = score
            best = cand
    return best if best_score >= _SOURCE_MIN_SCORE else None
...
def _dump_layout(layout: dict, asis_image: Path, template_images: list[Path],
                 dump_dir: Path,
                 matched_source: Path | None = None) -> Path:
...
def convert(captures_dir: Path, templates_dir: Path,
            ...
            frontend_dir: Path | None = None) -> dict[str, Any]:
    ...
    for img in asis_imgs:
        print(f"  → {img.name}")
        source_path = _match_source(img.stem, source_index)
        if source_path is not None:
            matched += 1
