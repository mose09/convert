...
- business_summary: 비즈니스 관점 1줄 요약 (예: "FAB 변경 시 기본
  파라미터 재로드").
- required_fields: 이 trigger 가 onSave / onSubmit 등 저장/제출 흐름
  일 때, handler chain 안에서 **필수 입력 검증** 을 받는 field 이름
  list (영문 — input 의 id / name 우선). 검증 패턴 예시:
  ``if (X === '' || X === null) errorList.push('[X]')`` / ``isNull(X)``
  / ``!X`` / ``X.length === 0`` / 커스텀 validation 라이브러리 호출.
  저장/제출 흐름이 아니거나 검증 없으면 빈 배열. parser 가 이미 잡은
  field 들도 같이 출력해도 OK (호출자가 머지).
...
**반환 schema (JSON 만)**:
{
  ...
  "business_summary": "...",
  "required_fields": ["DATA_TYPE", "EQ_ID"]
}
"""
