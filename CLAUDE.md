...
| `@Inject SqlSession simpleSqlSession` 같이 사내 wrapper 변수명을 가진 SQL 호출이 patterns.yaml 에 receiver 추가 안 하면 `_SQL_CALL_RE` 미매치 → SQL 누락 | default receivers 가 정확한 이름 (`sqlSession` 등) + 끝 suffix (`*Sql`/`*Dao`) 만 잡고 `simpleSqlSession` 같이 중간에 `Sql` 들어간 변형은 미스 | `_collect_sql_receiver_names_from_fields(autowired)` 자동 detection — 각 java 파일의 autowired 필드 중 type 이 `_SQL_SESSION_TYPES` (`SqlSession` / `SqlSessionTemplate` / `JdbcTemplate` / 등) 에 속하면 그 필드명을 SQL receiver 로 자동 추가. `_build_sql_call_re` / `_var_re` / `_ternary_re` 모두 factory 화 → `parse_java_file` 이 per-file regex 빌드 → `_collect_body_sql_calls` 에 inject. patterns.yaml 수동 등록 불필요. autowired 필드 키는 `name` / `type_simple` / `type_fqcn` (typo 주의). |
| `if(c){sqlId=NS+".x"} else{sqlId=NS+".y"} sqlSession.selectList(sqlId, p)` 형태 — sqlid 가 local var 로 분기 할당 후 호출 | 기존 매처는 literal / `var+"sfx"` / `var+(c?"a":"b")` 만 처리. local 변수 참조 시 호출 라인 자체에는 sqlid 정보 없어 매치 0건 | `_SQL_CALL_LOCALVAR_RE` 신규 — 1st arg 가 bare identifier 면 매치. `_collect_body_sql_calls` 가 method body 내 `varname = RHS;` assignment 모두 검색 + `_enumerate_sqlids_from_rhs(rhs, ns_consts)` 로 RHS 의 literal / `var+"sfx"` / `var+(c?"a":"b")` 양 분기 enumerate → 모든 후보 sqlid emit. local-var 가 method param 이거나 외부 source 면 assignment 못 찾아 자연스럽게 skip |
| `sqlSession.selectList(NAMESPACE + name, param)` 같이 prefix 와 suffix 가 둘 다 변수 (suffix 는 local var 로 분기 할당) 인 경우 미스 | shape-별 매처 (literal / var+suffix / ternary / localvar) 4 개로 누적 패치하다 보니 `var + var` 같은 새 변형 등장할 때마다 또 매처 추가하는 whack-a-mole | **통합 evaluator 로 architecture 교체**. ``_SQL_CALL_HEAD_RE`` (head 매칭만) + ``_extract_first_arg`` (balanced-paren 으로 1st arg 추출) + ``_eval_string_expr(expr, body, ns_consts)`` (symbolic evaluator). 지원: 리터럴 / 식별자 (ns_constants 우선 + body assignment 추적) / `a + b` (cartesian product) / `cond ? a : b` (양 분기 합집합) / 외부 괄호. 임의 깊이 / 임의 조합 자동 처리. 기존 4 매처 (`_SQL_CALL_VAR_RE` / `_TERNARY_RE` / `_LOCALVAR_RE`) 는 evaluator 가 우월히 cover 하므로 ``_collect_body_sql_calls`` 에서 사용 중단 (모듈 레벨 유지는 backward compat). 10/10 단위 + 5 mock 회귀 PASS. 무한 재귀 / explosion guard (`depth>6` / `cartesian>32`) 로 over-report 차단 |
| 메서드 안 ``String sqlId = "literal";`` 같은 local declaration 이 ``ns_constants`` 에 leak → evaluator 의 식별자 lookup 이 첫 값만 반환하고 body 분기 할당들 무시 | ``_extract_ns_constants`` 가 raw text 전체에서 ``String name = "value"`` 패턴을 모두 잡아 method body 내부 변수까지 namespace 상수로 등록 | ``_extract_ns_constants(content, method_ranges=...)`` 시그니처 확장 — method body 범위 안의 매치는 무시. ``parse_java_file`` 에서 ``methods = _extract_method_bodies(...)`` 먼저 실행 → ``method_ranges`` 계산 → ``_extract_ns_constants(raw, method_ranges)`` 호출 순서. PR #47 saveDpPubNotiInfo 의 body_sql_calls=2 vs 14 미해결 이슈도 같은 root cause 일 가능성 매우 높음 |
| 사내 wrapper 클래스 ``new QueryFloaterStream(sqlSession, NAMESPACE + "id", param)`` 같이 constructor 호출에 SqlSession + sqlid 를 넘기는 형태 미인식 (대용량 streaming 조회용 패턴) | head_re 가 ``receiver.op(`` 만 매치하고 ``new ClassName(`` 은 미지원 | ``_build_sql_constructor_head_re(sql_session_vars)`` 신규 — autowired SqlSession 필드 이름들로 ``\bnew \w+(<sqlSessionVar>,`` 매처 빌드. PR #76 의 자동 detection 인프라 재활용 (patterns.yaml 등록 불필요). ``_extract_arg_at(text, start, initial_depth=1)`` helper 로 2nd arg 추출 → ``_eval_string_expr`` 로 평가 → 기존 emit 로직 재사용. op 필드에 클래스명 (``QueryFloaterStream``) 보존, ``resolved_from=constructor:<cls>`` 표기. 1st-arg=SqlSession 컨벤션 가정 (사내 99% 케이스). 컨벤션 다른 wrapper 는 별도 처리 필요 |
